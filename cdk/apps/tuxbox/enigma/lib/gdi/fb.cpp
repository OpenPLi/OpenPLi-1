...
{
	// eDebug("fbClass::Box: pos=%dx%d size=%dx%d cols=%d,%d\n", x,y, width, height, color, backcolor);
	if (width<=2 || locked)
		return;
...
{
	// eDebug("fbClass::NBox: pos=%dx%d size=%dx%d color=%d\n", x,y, width, height, color);
	if (locked)
		return;
	int offset=y*stride+x/2;
	int halfwidth=width/2;
...
{
	// eDebug("fbClass::VLine: pos=%dx%d height=%d color=%d\n", x,y, sy, color);
	if (locked)
		return;
	int offset=y*stride+x/2;
	while (sy--)
...
{
	// allow setting colormaps when we are locked... On unlcok we restore our colormap anyway.
	// if (locked)
	// {
		// eDebug("fbClass: locked, not setting colormap");
		// return;
	// }

	if (map == NULL)
	{
		// eDebug("fbClass: setting palette to default");
		map = &cmap;
	}

	// eDebug("fbClass: setting palette");

...
class eSkin
{
	...
	std::map<eString, eString> values;
...
public:
	...
	eString queryValue(const eString &name, const eString &d) const;
...
#define TRY_ASSIGN(v, t, n) \
  v =(t*)search(n);

#define ASSIGN_MULTIPLE(v, t, n) \
  v = new t; searchMultiple(n, *v);

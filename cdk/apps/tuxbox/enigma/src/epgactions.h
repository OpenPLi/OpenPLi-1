...
struct epgSelectorActions
{
	...
	eAction addDVRTimerEvent, addNGRABTimerEvent, addSwitchTimerEvent,
		removeTimerEvent, showExtendedInfo, searchEPG, prevService, nextService; // EPG search ... added "searchEPG"
	epgSelectorActions()
		...
		prevService(map, "prevService", _("switch to epg of previous service") , eAction::prioDialog ),
		nextService(map, "nextService", _("switch to epg of next service") , eAction::prioDialog )
	{
	}

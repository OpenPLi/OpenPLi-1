#this class fills PR with the highest committed svn rev, found in SRC_URI
...
python __anonymous () {
	...
	if newpr:
		...
		if not pr or pr == '':
			prname = 'PR'
			pr = bb.data.getVar(prname, d, 1)
		# we need a fixed fake SRCDATE to avoid checking out and
		# overwriting the svn tarball every day, when nothing changed
		bb.data.setVar('SRCDATE', 'now', d)
		prval = 'r' + str(newpr)
		if pr and pr != '' and pr != 'r0':
			prval = prval + pr
		bb.data.setVar(prname, prval, d)
		bb.data.setVar('SRC_URI', ' '.join(newurls), d)
}
